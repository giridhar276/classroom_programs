...
def assert_number_of_rows_in_table(my_table, expected_num_of_rows):
    ...
    if expected_num_of_rows != actual_num_rows:
        raise AssertionError('The number of row did not match. The actual number is: {} and the expected\
        number is {}'.format(str(actual_num_rows), str(expected_num_of_rows)))
...
def assert_row_contains_text(my_table, text_to_check, row_number):
    ...
    if text_to_check not in row_text:
        raise AssertionError('The text {} is not in row {}'.format(text_to_check, row_number))
    else:
        print('The text {} is found in row {}'.fomat(text_to_check, row_number))
...
def assert_col_in_row_contains_text(my_table, text_to_check, row_number=0, col_number=0):
    ...
    if text_to_check not in col_text:
        raise AssertionError('The text {} is not in row {} column {}'.format(text_to_check, str(row_number), str(col_number)))
    else:
        print('The text {} is in row {} and column {}'.format(text_to_check, str(row_number), str(col_number)))
...
assert_col_in_row_contains_text(table, 'Alfreds Futterkiste', row_number=2, col_number=1)
